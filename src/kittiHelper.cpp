...
int main(int argc, char** argv)
{
    ...
    // /data/KITTI/odometry/sequences/00/times.txt
    std::string timestamp_path = "sequences/" + sequence_number + "/times.txt";
    std::ifstream timestamp_file(dataset_folder + timestamp_path, std::ifstream::in);
    std::cout << "Reading timestamp " << sequence_number << " from " << dataset_folder + timestamp_path << '\n';
    ...
    std::cout << "Reading gt " << sequence_number << " from " << dataset_folder + ground_truth_path << '\n';
    // 新建一个bag对象
    rosbag::Bag bag_out;
    if (to_bag)
    ...
    R_transform << 0, 0, 1, -1, 0, 0, 0, -1, 0; // odometry里L2C
    ...
    while (std::getline(timestamp_file, line) && ros::ok())
    {
        ...
        for (std::size_t i = 0; i < 3; ++i)
        {
            ...
            {
                std::getline(pose_stream, s, ' '); // 以空格为分隔符
                gt_pose(i, j) = stof(s);
            }
        }
